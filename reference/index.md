# Package index

## All functions

- [`checkSurveyOK()`](https://einarhjorleifsson.github.io/icesDatras/reference/checkSurveyOK.md)
  : Check that a survey name is in the database
- [`checkSurveyYearOK()`](https://einarhjorleifsson.github.io/icesDatras/reference/checkSurveyYearOK.md)
  : Check that a survey and year combination is in the database
- [`checkSurveyYearQuarterOK()`](https://einarhjorleifsson.github.io/icesDatras/reference/checkSurveyYearQuarterOK.md)
  : Check that a survey, year and quarter combination is in the database
- [`getCAdata()`](https://einarhjorleifsson.github.io/icesDatras/reference/getCAdata.md)
  : Get Age-Based Data
- [`getCPUEAge()`](https://einarhjorleifsson.github.io/icesDatras/reference/getCPUEAge.md)
  : Get CPUE per Age per Haul per Hour for a given Survey, year and
  Quarter
- [`getCPUELength()`](https://einarhjorleifsson.github.io/icesDatras/reference/getCPUELength.md)
  : Get CPUE per Length per Haul per Hour for a given Survey, year and
  Quarter
- [`getCatchWgt()`](https://einarhjorleifsson.github.io/icesDatras/reference/getCatchWgt.md)
  : Get Catch Weights
- [`getDATRAS()`](https://einarhjorleifsson.github.io/icesDatras/reference/getDATRAS.md)
  [`getDATRASCatched()`](https://einarhjorleifsson.github.io/icesDatras/reference/getDATRAS.md)
  : Get Any DATRAS Data
- [`getDatrasDataOverview()`](https://einarhjorleifsson.github.io/icesDatras/reference/getDatrasDataOverview.md)
  : Summarize Data Availability
- [`getDatrasFieldList()`](https://einarhjorleifsson.github.io/icesDatras/reference/getDatrasFieldList.md)
  : Get Datras field list with column classes and mapping to old naming
  schema
- [`getDatrasSchema()`](https://einarhjorleifsson.github.io/icesDatras/reference/getDatrasSchema.md)
  : Get the DATRAS field/type schema
- [`getDatrasUnaggregated()`](https://einarhjorleifsson.github.io/icesDatras/reference/getDatrasUnaggregated.md)
  : Download unaggregated DATRAS survey data
- [`getFlexFile()`](https://einarhjorleifsson.github.io/icesDatras/reference/getFlexFile.md)
  : Get Flex File
- [`getHHdata()`](https://einarhjorleifsson.github.io/icesDatras/reference/getHHdata.md)
  : Get Haul Data
- [`getHLdata()`](https://einarhjorleifsson.github.io/icesDatras/reference/getHLdata.md)
  : Get Length-Based Data
- [`getIndices()`](https://einarhjorleifsson.github.io/icesDatras/reference/getIndices.md)
  : Get Survey Indices
- [`getLTassessment()`](https://einarhjorleifsson.github.io/icesDatras/reference/getLTassessment.md)
  : Get Litter assessment output
- [`getSurveyList()`](https://einarhjorleifsson.github.io/icesDatras/reference/getSurveyList.md)
  : Get a List of All Surveys
- [`getSurveyYearList()`](https://einarhjorleifsson.github.io/icesDatras/reference/getSurveyYearList.md)
  : Get a List of Survey Years
- [`getSurveyYearQuarterList()`](https://einarhjorleifsson.github.io/icesDatras/reference/getSurveyYearQuarterList.md)
  : Get a List of Quarters
- [`get_datras_unaggregated_data()`](https://einarhjorleifsson.github.io/icesDatras/reference/get_datras_unaggregated_data.md)
  : Download unaggregated DATRAS survey data (Deprecated)
- [`icesDatras`](https://einarhjorleifsson.github.io/icesDatras/reference/icesDatras.md)
  [`icesDatras-package`](https://einarhjorleifsson.github.io/icesDatras/reference/icesDatras.md)
  : DATRAS Trawl Survey Database Web Services
